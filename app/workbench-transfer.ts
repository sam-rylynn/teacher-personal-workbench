...
export function parseDelimitedText(text: string): string[][] {
  ...
  // Choose one delimiter for the whole file from the first logical row.
  // Spreadsheet pastes are tab-separated, even when a cell contains commas.
  const separators = new Set<string>();
  let detectingQuotes = false;
  for (let index = 0; index < source.length; index += 1) {
    const char = source[index];
    if (char === '"') {
      if (detectingQuotes && source[index + 1] === '"') index += 1;
      else detectingQuotes = !detectingQuotes;
    } else if (!detectingQuotes) {
      if (char === "\n" || char === "\r") break;
      if (char === "," || char === "\t" || char === ";") separators.add(char);
    }
  }
  const separator = separators.has("\t") ? "\t" : separators.has(",") ? "," : ";";
  for (let index = 0; index < source.length; index += 1) {
    const char = source[index];
    if (inQuotes) {
      if (char === '"') {
  ...
  if (inQuotes) throw new Error("有单元格的引号没有结束，请检查后重新导入。");
  row.push(cell.trim());
  if (row.some((value) => value.length > 0)) rows.push(row);
  return rows;
}
...
function splitHeader(rows: string[][], requiredColumns: string[]): { header: string[]; body: string[][] } | null {
  ...
  // A data cell may itself say “教室” or “考试”; one matching word is not a header.
  const looksLikeHeader = first.filter((cell) => requiredColumns.includes(cell)).length >= 2;
...
function columnIndex(header: string[], aliases: string[], fallback: number): number {
  ...
  return header.length ? -1 : fallback;
}

function validateImportHeader(header: string[], columns: Record<string, readonly string[]>, required: readonly string[]): ImportIssue[] {
  if (!header.length) return [];
  const issues: ImportIssue[] = [];
  for (const [key, aliases] of Object.entries(columns)) {
    const count = header.filter((cell) => aliases.includes(cell)).length;
    if (count > 1) issues.push({ rowNumber: 1, field: "表头", message: `“${aliases[0]}”列出现多次，请只保留一列。` });
    if (count === 0 && required.includes(key)) issues.push({ rowNumber: 1, field: "表头", message: `缺少“${aliases[0]}”列，请检查表头。` });
  }
  return issues;
}
...
const ASSESSMENT_COLUMNS = {
  ...
  classAverage: ["班级平均分", "班级均分", "均分", "平均分"],
} as const;
...
export function planAssessmentImport(
  ...
  let rows: string[][];
  try { rows = parseDelimitedText(text); } catch (error) {
    return { entries: [], issues: [{ rowNumber: 0, field: "内容", message: (error as Error).message }], newStudentCount: 0 };
  }
  const issues: ImportIssue[] = [];
  const entries: AssessmentImportEntry[] = [];
  const split = splitHeader(rows, Object.values(ASSESSMENT_COLUMNS).flat());
  if (!split || split.body.length === 0) {
    return { entries, issues: [{ rowNumber: 0, field: "内容", message: "没有找到可以导入的内容，请检查表头并至少填写一行记录。" }], newStudentCount: 0 };
  }
  const headerIssues = validateImportHeader(split.header, ASSESSMENT_COLUMNS, ["name", "className", "title", "occurredOn", "score", "rank", "cohortSize"]);
  if (headerIssues.length) return { entries, issues: headerIssues, newStudentCount: 0 };
  ...
  split.body.forEach((cells, bodyIndex) => {
    ...
    if (split.header.length && cells.length !== split.header.length) return fail("内容", "这一行的列数与表头不一致，请检查是否有漏列或多余的分隔符。");
    ...
    const maxScoreRaw = cells[index.maxScore] ?? "";
    const maxScore = maxScoreRaw === "" ? options.defaultMaxScore ?? 100 : parseNumberCell(maxScoreRaw);
    ...
    const classAverageRaw = cells[index.classAverage] ?? "";
    const classAverage = classAverageRaw === "" ? 0 : parseNumberCell(classAverageRaw);

    if (!name) return fail("姓名", "缺少学生姓名。");
    if (!className) return fail("班级", "缺少班级。");
    if (!title) return fail("测评", "缺少测评名称。");
    if (!isValidDate(occurredOn)) return fail("日期", `日期“${cells[index.occurredOn] ?? ""}”格式不正确，请按“2026-10-12”填写。`);
    if (score === null) return fail("成绩", "成绩请填写数字，如 86.5。");
    if (maxScore === null || !Number.isFinite(maxScore)) return fail("满分", "满分请填写数字，如 100。");
    if (classAverage === null) return fail("班级均分", "班级均分请填写数字，如 79.5。");
    if (maxScore <= 0 || score < 0 || score > maxScore) return fail("成绩", `成绩 ${score} 超出 0 到满分 ${maxScore} 的范围。`);
    if (rank === null || !Number.isInteger(rank)) return fail("排名", "排名请填写 1、2、3 这样的整数。");
    if (cohortSize === null || !Number.isInteger(cohortSize) || cohortSize < 1) return fail("参考人数", "参考人数请填写大于 0 的整数。");
    ...
    const fileKey = JSON.stringify([name, className, subject, title, occurredOn]);
    if (seenInFile.has(fileKey)) return fail("重复", "与本次导入内容中的另一行重复。");
    seenInFile.add(fileKey);

    const existing = findStudentByIdentity(students, name, className);
    if (existing?.assessments.some((assessment) => assessment.subject === subject && assessment.title === title && assessment.occurredOn === occurredOn)) {
      return fail("重复", "该学生已有名称和日期都相同的测评记录。");
    }

    const studentKey = JSON.stringify([name, className]);
...
export function applyAssessmentImportPlan(
  ...
  const byIdentity = new Map(draft.students.map((student) => [JSON.stringify([student.name, student.className]), student]));
  ...
  plan.entries.forEach((entry, index) => {
    const key = JSON.stringify([entry.studentName, entry.className]);
    ...
    if (student.assessments.some((assessment) => assessment.subject === entry.record.subject && assessment.title === entry.record.title && assessment.occurredOn === entry.record.occurredOn)) return;
...
export function planLessonImport(text: string, lessons: readonly LessonSession[], workbench?: WorkbenchData): LessonImportPlan {
  let rows: string[][];
  try { rows = parseDelimitedText(text); } catch (error) {
    return { entries: [], issues: [{ rowNumber: 0, field: "内容", message: (error as Error).message }] };
  }
  const issues: ImportIssue[] = [];
  const entries: LessonImportEntry[] = [];
  const split = splitHeader(rows, Object.values(LESSON_COLUMNS).flat());
  if (!split || split.body.length === 0) {
    return { entries, issues: [{ rowNumber: 0, field: "内容", message: "没有找到可以导入的内容，请检查表头并至少填写一行记录。" }], };
  }
  const headerIssues = validateImportHeader(split.header, LESSON_COLUMNS, ["title", "subject", "className", "date", "start", "end", "room"]);
  if (headerIssues.length) return { entries, issues: headerIssues };
  ...
  split.body.forEach((cells, bodyIndex) => {
    ...
    const start = (cells[index.start] ?? "").padStart(5, "0");
    const end = (cells[index.end] ?? "").padStart(5, "0");
    const room = cells[index.room] ?? "";
    const preparation = cells[index.preparation] ?? "";
    const reminderRaw = cells[index.reminder] ?? "";

    if (!title) return fail("标题", "缺少课程名称。");
    if (!subject) return fail("学科", "缺少学科。");
    if (!className) return fail("班级", "缺少班级。");
    if (!isValidDate(date)) return fail("日期", `日期“${cells[index.date] ?? ""}”格式不正确，请按“2026-10-13”填写。`);
    if (!TIME_PATTERN.test(start)) return fail("开始时间", `开始时间“${start}”格式不正确，请按“08:55”填写。`);
    if (!TIME_PATTERN.test(end)) return fail("结束时间", `结束时间“${end}”格式不正确，请按“09:40”填写。`);
    ...
    const overlaps = (lesson: Omit<LessonSession, "id">) => lesson.status !== "已取消" && new Date(lesson.startsAt).getTime() < new Date(endsAt).getTime() && new Date(lesson.endsAt).getTime() > new Date(startsAt).getTime();
    const scheduledLessons = workbench ? expandLessonsForRange(workbench, date, date, `${date}T00:00:00+08:00`) : lessons;
    if (scheduledLessons.some(overlaps)) {
      return fail("时间冲突", "这个时段已有课程，请检查后再导入。");
    }
    if (entries.some((entry) => overlaps(entry.record))) return fail("时间冲突", "与本次导入的另一节课时间重叠，请检查后再导入。");

    const reminderMinutes = reminderRaw === "" ? null : /^\d+(?:\s*分钟)?$/.test(reminderRaw) ? Number.parseInt(reminderRaw, 10) : NaN;
    if (
      reminderRaw !== "" &&
...
export function applyLessonImportPlan(draft: WorkbenchData, plan: LessonImportPlan, now: string): number {
  ...
  let added = 0;
  plan.entries.forEach((entry, index) => {
    const date = entry.record.startsAt.slice(0, 10);
    const scheduledLessons = expandLessonsForRange(draft, date, date, `${date}T00:00:00+08:00`);
    if (scheduledLessons.some((lesson) => lesson.status !== "已取消" && new Date(lesson.startsAt).getTime() < new Date(entry.record.endsAt).getTime() && new Date(lesson.endsAt).getTime() > new Date(entry.record.startsAt).getTime())) return;
    draft.lessons.push({
      ...entry.record,
...
function mobileString(value: unknown, label: string, maxLength = MOBILE_VIEW_MAX_BYTES): string {
...
function mobileOptionalString(value: unknown, label: string, maxLength = MOBILE_VIEW_MAX_BYTES): string | undefined {
...
function mobileIso(value: unknown, label: string): string {
  ...
  if (!/^\d{4}-\d{2}-\d{2}T(?:[01]\d|2[0-3]):[0-5]\d:[0-5]\d(?:\.\d{1,3})?(?:Z|[+-](?:[01]\d|2[0-3]):[0-5]\d)$/.test(text) || !isValidDate(text.slice(0, 10)) || Number.isNaN(new Date(text).getTime())) throw new Error(`${label}不是有效时间。`);
  return text;
}
...
function rebuildMobileStudent(value: unknown, index: number): MobileStudentSummary {
  ...
  const student: MobileStudentSummary = {
    id: mobileString(record.id, "学生编号", 120),
    name: mobileString(record.name, "学生姓名"),
    className: mobileString(record.className, "班级"),
    ...
    issueTitle: record.issueTitle === null ? null : mobileString(record.issueTitle, "近期问题"),
    issueStatus,
  };
  if ((student.latestScore === null) !== (student.latestMaxScore === null) || (student.currentRank === null) !== (student.cohortSize === null)) throw new Error("学生成绩或排名内容不完整。");
  if (student.latestMaxScore !== null && (student.latestMaxScore <= 0 || student.latestScore! < 0 || student.latestScore! > student.latestMaxScore)) throw new Error("学生成绩超出满分范围。");
  if (student.currentRank !== null && (!Number.isInteger(student.currentRank) || !Number.isInteger(student.cohortSize) || student.currentRank < 1 || student.currentRank > student.cohortSize!)) throw new Error("学生排名或参考人数不正确。");
  if (student.rankDelta !== null && !Number.isInteger(student.rankDelta)) throw new Error("排名变化不正确。");
  if ((student.issueTitle === null) !== (student.issueStatus === null)) throw new Error("学生问题内容不完整。");
  return student;
}
...
function rebuildMobileLesson(value: unknown, index: number): LessonSession {
  ...
  const lesson: LessonSession = {
    ...
    title: mobileString(record.title, "课次标题"),
    subject: mobileString(record.subject, "学科"),
    className: mobileString(record.className, "班级"),
    startsAt: mobileIso(record.startsAt, "课次开始时间"),
    endsAt: mobileIso(record.endsAt, "课次结束时间"),
    room: typeof record.room === "string" && record.room.length <= MOBILE_VIEW_MAX_BYTES ? record.room : (() => { throw new Error("课次地点不正确。"); })(),
    preparation: typeof record.preparation === "string" && record.preparation.length <= MOBILE_VIEW_MAX_BYTES ? record.preparation : (() => { throw new Error("备课清单不正确。"); })(),
    status: record.status,
    reminderMinutesBefore: record.reminderMinutesBefore === null ? null : mobileNumber(record.reminderMinutesBefore, "课前提醒", { integer: true, min: 1, max: MAX_LESSON_REMINDER_MINUTES }),
  };
  if (new Date(lesson.endsAt).getTime() <= new Date(lesson.startsAt).getTime()) throw new Error("课程结束时间应晚于开始时间。");
  return lesson;
}
...
function rebuildMobileTask(value: unknown, index: number): WorkbenchTask {
  ...
  const relatedLabel = mobileOptionalString(record.relatedLabel, "事项关联");
  const completedAt = record.completedAt === undefined ? undefined : mobileIso(record.completedAt, "完成时间");
  const task: WorkbenchTask = {
    id: mobileString(record.id, "事项编号", 160),
    category: record.category,
    title: mobileString(record.title, "事项标题"),
  ...
  if (task.reminderAt && new Date(task.reminderAt).getTime() > new Date(task.dueAt).getTime()) throw new Error("事项提醒时间不能晚于截止时间。");
  return task;
}
...
function rebuildMobileSnapshot(value: unknown): MobileReadOnlySnapshot {
  ...
  const snapshot: MobileReadOnlySnapshot = {
    ...
    workbenchName: mobileString(record.workbenchName, "工作台名称"),
  ...
  for (const items of [snapshot.priorityStudents, snapshot.upcomingLessons, snapshot.openTasks]) {
    if (new Set(items.map((item) => item.id)).size !== items.length) throw new Error("手机查看文件包含重复记录。");
  }
  const summary = snapshot.summary;
  if ([summary.studentsWithConfirmedAssessments, summary.rankImproved, summary.rankDeclined, summary.scoreImproved, summary.scoreDeclined, summary.issuesPending, summary.issuesWatching, summary.homeSchoolFollowUps].some((count) => count > summary.totalStudents)) throw new Error("手机看板的学生人数不一致。");
  return snapshot;
}
...
export function serializeMobileViewFile(snapshot: MobileReadOnlySnapshot, exportedAt: string): string {
  const text = JSON.stringify({
    ...
    exportedAt: mobileIso(exportedAt, "导出时间"),
    snapshot: rebuildMobileSnapshot(snapshot),
  } satisfies MobileViewFileV1, null, 2);
  if (new TextEncoder().encode(text).byteLength > MOBILE_VIEW_MAX_BYTES) throw new Error("手机查看内容超过 512KB，请减少本次展示的内容后重新生成。");
  return text;
}
...
export function parseMobileViewFile(text: string, current: MobileReadOnlySnapshot | null = null): MobileViewImportResult {
  ...
  const warnings = current && new Date(envelope.snapshot.generatedAt).getTime() < new Date(current.generatedAt).getTime()
    ? ["这个文件比手机现在显示的内容更早，导入后会回到较早的内容。"]
    : [];
...
export function saveImportedMobileView(storage: StorageLike, snapshot: MobileReadOnlySnapshot): { ok: true } | { ok: false; message: string } {
  try {
    ...
    return { ok: false, message: "手机内容暂时无法保存，请检查浏览器存储空间后重试。" };
  }
}
...
export function loadImportedMobileView(storage: StorageLike): { snapshot: MobileReadOnlySnapshot | null; warning?: string } {
  try {
    ...
    return { snapshot: null, warning: "手机内容无法读取，请重新导入电脑生成的文件。" };
  }
}
...
export function buildIcsCalendar(data: WorkbenchData, now: string): string {
  ...
  for (const lesson of effectiveLessons) {
    ...
    lines.push(`UID:${escapeIcsText(lesson.id)}@teacher-workbench`);
  ...
  for (const task of data.tasks) {
    ...
    lines.push("BEGIN:VEVENT");
    lines.push(`UID:${escapeIcsText(task.id)}@teacher-workbench`);
    lines.push(`DTSTAMP:${stamp}`);
    lines.push(`SEQUENCE:${data.meta.revision}`);
    lines.push(`LAST-MODIFIED:${stamp}`);
    ...
    if (task.reminderAt) {
      ...
      if (Number.isFinite(remindAt) && remindAt <= due.getTime()) {
        lines.push("BEGIN:VALARM");
        lines.push(`TRIGGER;VALUE=DATE-TIME:${toIcsUtc(task.reminderAt)}`);
        lines.push("ACTION:DISPLAY");
 ...
 * inside [reminderAt, dueAt), with a one-minute minimum window so polling does
 * not miss a reminder set exactly at the deadline. Delivered keys are kept
 * by the caller
...
export function computeDueReminders(data: WorkbenchData, nowIso: string, delivered: ReadonlySet<string>): DueReminder[] {
  ...
  for (const task of data.tasks) {
    ...
    if (remindAt <= dueAt && now >= remindAt && now < Math.max(dueAt, remindAt + 60_000) && !delivered.has(key)) {
      due.push({
        key,
        kind: "事项",
        title: task.title,
...
function isInboxItem(value: unknown): value is InboxItem {
  ...
  return (
    Object.keys(item).every((key) => ["id", "text", "category", "createdAt"].includes(key)) &&
    typeof item.id === "string" && item.id.trim().length > 0 && item.id.length <= 120 &&
    typeof item.text === "string" && item.text.trim().length > 0 &&
    INBOX_CATEGORIES.includes(item.category) &&
    typeof item.createdAt === "string" && isValidDate(item.createdAt.slice(0, 10)) && !Number.isNaN(new Date(item.createdAt).getTime())
  );
}
...
export function parseInbox(text: string): InboxItem[] {
  if (new TextEncoder().encode(text).byteLength > MOBILE_VIEW_MAX_BYTES) throw new Error("速记内容过多，请分批复制导入。");
  let parsed: unknown;
  try {
    parsed = JSON.parse(text);
  } catch {
    throw new Error("没有识别到手机速记。请先在手机点击“复制速记”，再粘贴到这里。");
  }
  const items = Array.isArray(parsed)
    ? parsed
    : (typeof parsed === "object" && parsed !== null && (parsed as { kind?: unknown }).kind === "teacher-workbench-inbox" && (parsed as { version?: unknown }).version === 1 && Array.isArray((parsed as { items?: unknown }).items)
        ? (parsed as { items: unknown[] }).items
        : null);
  if (!items) throw new Error("没有识别到速记内容。");
  if (!items.length) throw new Error("没有识别到速记内容。");
  if (!items.every(isInboxItem)) throw new Error("部分速记内容不完整，请从手机重新复制；本次没有导入任何速记。");
  const unique = new Map<string, InboxItem>();
  for (const item of items) {
    const existing = unique.get(item.id);
    if (existing && (existing.text !== item.text || existing.category !== item.category || existing.createdAt !== item.createdAt)) throw new Error("速记中有编号相同但内容不同的记录，请检查后重新复制。");
    unique.set(item.id, { id: item.id, text: item.text.trim(), category: item.category, createdAt: item.createdAt });
  }
